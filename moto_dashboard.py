...
DEFAULT_THR = {
    "motor_temp":{"warn":80,"crit":100,"dir":"high"},"mcu_temp":{"warn":70,"crit":85,"dir":"high"},
    "board_temp_bms1":{"warn":60,"crit":75,"dir":"high"},"board_temp_bms2":{"warn":60,"crit":75,"dir":"high"},
    "curr_mcu":{"warn":250,"crit":350,"dir":"high"},"soc_bms1":{"warn":15,"crit":10,"dir":"low"},
...
def init_db():
    with get_engine().connect() as con:
        con.execute(text("""CREATE TABLE IF NOT EXISTS sessions (
            ...
            track TEXT, weather TEXT, notes TEXT, upload_time TEXT, row_count INTEGER, duration_s REAL)"""))
...
def parse_time_col(series):
    try:
        dt = pd.to_datetime(series, utc=True, infer_datetime_format=True)
        return (dt - dt.iloc[0]).dt.total_seconds().values
    except:
        pass
    nums = pd.to_numeric(series, errors="coerce")
    if nums.notna().sum() < len(series)*0.5: return np.arange(len(series))/50.0
...
def parse_csv(raw_bytes):
    ...
    for col in df.columns:
        c = col.lower()
        if "timestamp" in c:                               rename[col]="timestamp_utc"
    ...
    df["t"] = parse_time_col(df["timestamp_utc"]) if "timestamp_utc" in df.columns else np.arange(len(df))/50.0
    return df
...
def bust_cache(): load_sessions.clear(); load_signals.clear()
...
def detect(df, thr):
    ...
    for col,lbl in [("mcu_errors","MCU"),("bms1_errors","BMS1"),("bms2_errors","BMS2")]:
        ...
        bad = vals.notna() & ~vals.isin(["0","0.0","0x0","0x0000","nan",""])
        if bad.sum()>0:
            issues.append({"sev":"🔴 Critical","type":f"{lbl} Error Code",
                "desc":f'Codes: {", ".join(vals[bad].unique()[:5].tolist())}',
    ...
    for ch,cfg in thr.items():
        ...
        for mask,sev,tv in [(c_mask,"🔴 Critical",cfg["crit"]),(w_mask,"🟡 Warning",cfg["warn"])]:
            ...
            issues.append({"sev":sev,"type":f'{CHANNELS.get(ch,ch)} {"Critical" if "🔴" in sev else "Warning"}',
...
def build_context(df, issues, srow):
    lines = ["ELECTRIC MOTORCYCLE TELEMETRY SESSION SUMMARY",
             f"Session: {srow.get('name','?')} | Date: {srow.get('date','?')} | Rider: {srow.get('rider','?')} | Track: {srow.get('track','?')}",
             f"Duration: {df['t'].max():.0f}s | Samples: {len(df):,} | Weather: {srow.get('weather','?')}"]
    ...
    for iss in issues:
        lines.append(f"  [{iss['sev']}] {iss['type']} — {iss['desc']} @ T+{iss['t']:.1f}s")
    return "\n".join(lines)
...
def ask_claude(context, question, history):
    ...
    system = ("You are an expert electric motorcycle engineer and data analyst for an R&D team. "
    ...
    resp = requests.post("https://api.anthropic.com/v1/messages",
        ...
        json={"model":"claude-sonnet-4-20250514","max_tokens":1024,"system":system,"messages":msgs},timeout=30)
    data = resp.json()
...
def page_upload():
    ...
    if ok:
        ...
        with st.spinner("Saving…"):
            ...
            save_to_db((name,str(date),rider,track,weather,notes,datetime.now().isoformat(),len(df),dur), df)
            bust_cache()
        st.success(f"✅ '{name}' saved — {len(df):,} rows, {dur:.0f}s")
...
def page_sessions():
    ...
    for _,r in sess.iterrows():
        with st.expander(f"**{r['name']}** — {r['date']}  |  {r.get('track','') or '—'}  |  {r.get('rider','') or '—'}"):
            ...
            cc.write(f"**Duration:** {r['duration_s']:.0f}s")
...
def page_analyse(thr):
    ...
    sid, srow = opts[label]; df = load_signals(sid)

    st.subheader("Key Metrics")
    m1,m2,m3,m4,m5,m6 = st.columns(6)
    def vmax(c): v=num(df,c); return f"{v.max():.1f}" if not v.empty else "—"
    def vmin(c): v=num(df,c); return f"{v.min():.1f}" if not v.empty else "—"
    ...
    m6.metric("Duration",        f"{df['t'].max():.0f}s")

    st.subheader("⚠️ Anomaly Report")
    issues = detect(df, thr)
    if not issues: st.success("✅ No anomalies detected.")
    for iss in issues:
        color = "#c0392b" if "🔴" in iss["sev"] else "#e67e22"
        bg    = "#fdf0f0" if "🔴" in iss["sev"] else "#fef9e7"
        st.markdown(f'<div style="border-left:4px solid {color};background:{bg};padding:8px 14px;margin:4px 0;border-radius:4px">'
                    f'<b>{iss["sev"]} — {iss["type"]}</b><br>{iss["desc"]} &nbsp;|&nbsp; '
                    f'First at <b>T+{iss["t"]:.1f}s</b> &nbsp;|&nbsp; Duration: <b>{iss["dur"]:.1f}s</b></div>',
    ...
    if sel:
        ...
        t_range = st.slider("Time window (s)", 0.0, t_max, (0.0,t_max), step=0.5)
        ...
        fig.update_layout(height=480,hovermode="x unified",xaxis_title="Time (s)",
...
def page_compare(thr):
    ...
    with st.expander(f"⚠️ Anomalies — A ({len(ia)} issues)"):
        for iss in ia: st.markdown(f"**{iss['sev']}** {iss['type']} — {iss['desc']}")
        if not ia: st.success("No anomalies.")
    with st.expander(f"⚠️ Anomalies — B ({len(ib)} issues)"):
        for iss in ib: st.markdown(f"**{iss['sev']}** {iss['type']} — {iss['desc']}")
    ...
    for ch in sel:
        ...
        fig.add_trace(go.Scatter(x=sa["t"],y=num(sa,ch),name=f"A: {lbl_a[:30]}",
            line=dict(color="#1f77b4",width=1.8),hovertemplate=f"<b>A</b> {CHANNELS[ch]}: %{{y:.2f}}<extra></extra>"))
        fig.add_trace(go.Scatter(x=sb["t"],y=num(sb,ch),name=f"B: {lbl_b[:30]}",
            line=dict(color="#ff7f0e",width=1.8),hovertemplate=f"<b>B</b> {CHANNELS[ch]}: %{{y:.2f}}<extra></extra>"))
        if ch in thr:
        ...
        fig.update_layout(title=CHANNELS[ch],height=280,hovermode="x unified",
                          xaxis_title="Time (s)",margin=dict(t=35,b=30),legend=dict(orientation="h",y=-0.35))
        st.plotly_chart(fig,use_container_width=True)
...
def main():
    ...
    st.sidebar.divider()
    page=st.sidebar.radio("Go to",["📤 Upload Session","📋 All Sessions","📊 Analyse Session","🔄 Compare Sessions"])
    with st.sidebar.expander("⚙️ Alert Thresholds"):
        thr={}
        for ch,cfg in DEFAULT_THR.items():
            lbl=CHANNELS.get(ch,ch)
            w=st.number_input(f"{lbl} {'warn' if cfg['dir']=='high' else 'low warn'}",value=cfg["warn"],key=f"w{ch}")
            c=st.number_input(f"{lbl} {'crit' if cfg['dir']=='high' else 'low crit'}",value=cfg["crit"],key=f"c{ch}")
